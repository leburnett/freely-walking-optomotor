...
for typ_id = 1:n_data_types
    ...
    elseif data_type == "dist_data_delta"
        [pvals_delta, target_mean_delta, control_mean_delta] = dist_metric_tests(cond_data, cond_data_control, 2); % 3 metrics

    elseif data_type == "dist_dt"
        [pvals_dist_dt, target_mean_dist_dt, control_mean_dist_dt] = dist_metric_tests(cond_data, cond_data_control, 3); % 5 metrics
    end 
...
pvals = horzcat(pvals_fv, pvals_av,  pvals_cv,  pvals_dist, pvals_delta, pvals_dist_dt);

target_mean = horzcat(target_mean_fv, target_mean_av, target_mean_cv, target_mean_dist, target_mean_delta, target_mean_dist_dt);

control_mean = horzcat(control_mean_fv, control_mean_av, control_mean_cv, control_mean_dist, control_mean_delta, control_mean_dist_dt);
