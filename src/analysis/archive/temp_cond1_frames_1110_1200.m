%% temp_cond1_frames_1110_1200 — Heatmap of condition 1 metrics (frames 1110-1200)
%
% Same plot as analyse_acclim_off1 but using frames 1110-1200 from
% condition 1 (R1 and R2 averaged) instead of acclim_off1.
%
...
use_abs      = [false,      true,      false,       false];  % absolute value before averaging
...
%% 3 — Compute per-strain mean metrics from condition 1 (frames 1110-1200)

metrics_matrix = NaN(n_strains, n_metrics);
ctrl_sd = NaN(1, n_metrics);
...
for s = 1:n_strains
    ...
    for exp_idx = 1:n_exp
        for rep = 1:2
            rep_str = sprintf('R%d_condition_1', rep);
            ...
            for m = 1:n_metrics
                field = data_fields{m};
                if ~isfield(rep_data, field); continue; end
                raw = rep_data.(field);  % [n_flies x n_frames]
                if use_abs(m); raw = abs(raw); end
                n_frames = size(raw, 2);
                f1 = min(FRAME_START, n_frames);
                f2 = min(FRAME_END, n_frames);
                if f1 > f2; continue; end
                per_fly = nanmean(raw(:, f1:f2), 2);  %#ok<NANMEAN>
...
%% 4 — Compute z-scores from control
...
fprintf('\n=== Condition 1 (frames %d-%d) mean metrics per strain ===\n', FRAME_START, FRAME_END);
...
title(sprintf('Condition 1 (frames %d–%d) — difference from ES control', FRAME_START, FRAME_END), 'FontSize', 14);
