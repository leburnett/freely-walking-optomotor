    ...
    for c_idx = 1:numel(cond_fields)
        ...
        for sex_idx = 1:numel(sexes)
            ...
            for idx = 1:n_exp
                ...
                % Build condition label with condition number prefix
                if ~is_acclim && isfield(cond_data, 'optomotor_pattern')
                    % Extract condition number from field name (e.g. R1_condition_3 → 3)
                    cond_num_str = extractAfter(cond_field, 'condition_');
                    cond_label_str = sprintf('Cond%s - Pat%d Spd%d %ds', ...
                        cond_num_str, ...
