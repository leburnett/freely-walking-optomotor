function plot_timeseries_ES_T4T5()
% Protocol 10 - timeseries data - T4T5 Kir - ES Kir.
...
% Timeseries  / errorbar plot (tuning).
    ...
    for gp = [1,2]
        ...
        for idx2 = [3,2] %[2,6,10] % condition 
            ...
            if isfield(data, rep1_str)
                ...
                plot(mean_data_dwn, 'Color', col, 'LineWidth', 2);
    ...
    f.Position = [314   703   519   263];
    
    cfg = get_config();
    fig_save_folder = fullfile(cfg.figures, "examples", "p10_T4T5_ES_Kir");
