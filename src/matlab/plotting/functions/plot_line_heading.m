function plot_line_heading(data_path, save_figs, fig_save_path, mean_med)

    % Plot the average heading_angle over the course of the experiment. 
    ...
    vel_files = dir('*angvel*');
    % end 
    
    n_files = length(vel_files);
    
     % Load the data
    load(fullfile(vel_files(1).folder, vel_files(1).name), 'Log', 'trx');
    ...
    for idx = 1:n_files 
        ...
        % Load the data
        load(fullfile(vel_files(idx).folder, vel_files(idx).name), 'trx'); % add 'save_str' back here eventually. 

        n_flies = height(trx');

        for i = 1:n_flies
            plot(unwrap(trx(i).theta), 'Color', [0.5 0.5 0.5], 'LineWidth', 0.01); 
            hold on 
        end
    end 
    
    data = unwrap(trx(1).theta);
    ...
    for ii = 1:n_files
        % Load the data
        load(fullfile(vel_files(ii).folder, vel_files(ii).name), 'trx');

        n_flies = height(trx');

        for jj = 1:n_flies
            data = unwrap(trx(jj).theta);
            if numel(data)<min_length 
                continue
            else
                dtt = data(1:min_length)';
    ...
    title(strcat('Unwrapped heading angle - N = ', string(n_flies_total)))

    ylim([-30 50])
    % ylim([0 15])
    if save_figs 
            savefig(gcf, fullfile(fig_save_path, strcat('Heading_', save_str,'_', mean_med,'.fig')))
    end 
