function hFig = plot_fly_occupancy_quadrants_diff_phototaxis(DATA, entryIdx, varargin)
% Circular quadrant occupancy DIFFERENCE heatmap for phototaxis.
% Renders with the BAR direction at the TOP (12 o'clock).
%
% It computes fraction occupancy per quadrant in two epochs:
%   Early  : frames 1:300
%   Later  : frames 300:1200
% and plots Δ = Later − Early as wedge colors (blue=decrease, red=increase).
%
...
% One condition (as in your base function, R1+R2 pooled)
...
% Circle for drawing (same for all panels)
th = linspace(0, 2*pi, 512);
xc = C(1) + R*cos(th);
yc = C(2) + R*sin(th);
bar_pt_draw = C + R*[cos(pi/2), sin(pi/2)];  % 12 o'clock visual bar

% ---------- Figure ----------
% hFig = figure('Color','w','Name','Fly quadrant occupancy Δ (Later−Early)');
% tiledlayout(hFig,1,numel(condList),'TileSpacing','compact','Padding','compact');

% --- Helper to compute normalized quadrant occupancy for a given frame range
    function Q = quadrant_fraction(fr)
        cond = condList{1};
        [x1,y1] = fetch_xy(DATA, entryIdx, 'R1', cond, fr(1):fr(2));
        [x2,y2] = fetch_xy(DATA, entryIdx, 'R2', cond, fr(1):fr(2));
        x = [x1(:); x2(:)];
        y = [y1(:); y2(:)];
        ...
        % Classify by bar-centered dividers (in DATA frame)
        dx = x - C(1);  dy = y - C(2);
...
% Compute fractions for each epoch
Q_early = quadrant_fraction(epoch1);
Q_late  = quadrant_fraction(epoch2);

% Map to wedge order (bar-at-top draw frame)
vals_early = [Q_early(1,2), Q_early(1,1), Q_early(2,1), Q_early(2,2)];  % [Q1 Q2 Q3 Q4]
vals_late  = [Q_late(1,2),  Q_late(1,1),  Q_late(2,1),  Q_late(2,2) ];

% Difference Δ = Later − Early
dvals = vals_late - vals_early;

% Robust symmetric color limits around 0
amax = max(abs(dvals));
if amax <= 0
    clim_pair = [-0.25, 0.25]; % fallback
else
    if useSym
        % Optionally use robust percentile across abs values (degenerate with 4 values, but fine)
        a = sort(abs(dvals));
        a_pct = a(min(numel(a), max(1, round(pctMax/100 * numel(a)))));
        amax = max(amax, a_pct);
        clim_pair = [-amax, +amax];
    else
        clim_pair = [min(dvals), max(dvals)];
        if diff(clim_pair)==0, clim_pair = [-0.25, 0.25]; end
    end
end

% Build a simple blue-white-red diverging colormap
n = 256; h = n/2;
cmap = [linspace(0,1,h)', linspace(0,1,h)', ones(h,1); ...   % blue -> white
        ones(h,1),        linspace(1,0,h)', linspace(1,0,h)'];% white -> red

% ---------- Plot single circular difference heatmap ----------
ax = gca; hold(ax,'on'); axis(ax,'off'); %  axis(ax,'equal');
colormap(ax, cmap);
clim(ax, clim_pair);

% Draw four wedges with Δ values
nArc = 128;
for k = 1:4
...
title(ax, sprintf('Δ Occupancy: [%d:%d] − [%d:%d]', epoch2(1), epoch2(2), epoch1(1), epoch1(2)));
...
% ---- Filter: keep only points within [0, 250] box ----
