...
% If 'cond_idx' == 0, then it combines the data during the acclimatisation
...
% cond_data : array (size = [n_flies, n_frames_condition+interval])
    ...
    if cond_idx == 0 % ACCLIM 1 - OFF 
        ...
        for idx = 1:n_exp
            ...
            acclim_data = data(idx).acclim_off1.(data_type);
            nf_comb = size(cond_data, 2);
            
            if nf_comb ~= 0 % First exp
            ...
            cond_data = vertcat(cond_data, acclim_data);
        end 
