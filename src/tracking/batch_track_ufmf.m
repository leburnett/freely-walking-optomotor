...
function tracking_log = batch_track_ufmf(date_folder)
    ...
    cd(date_folder)
    % Split path on the correct separator depending on OS:
    % Mac/Linux uses '/', Windows uses '\'
    if contains(date_folder, '/')
        folder_parts = strsplit(date_folder, '/');   % Mac / Linux
    elseif contains(date_folder, '\')
        folder_parts = strsplit(date_folder, '\');   % Windows
    end
    ...
    if ~exist('base_calib', 'var')
        cfg = get_config();
        base_calib = load(cfg.calibration_file);
    end
    ...
    cfg = get_config();
    tracking_log_dir = fullfile(cfg.project_root, 'tracking_log');
    if ~isfolder(tracking_log_dir); mkdir(tracking_log_dir); end
    log_fname = fullfile(tracking_log_dir, strcat('Tracked_', date_str, '.mat'));
