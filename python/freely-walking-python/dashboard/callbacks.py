...
def register_callbacks(app, data_store):
    ...
    @app.callback(
        ...
        Input("pipeline-refresh-btn", "n_clicks"),
        Input("pipeline-interval", "n_intervals"),
        State("pipeline-registry-path", "value"),
    )
    def update_pipeline_table(n_clicks, n_intervals, registry_path):
        """Load pipeline registry and populate the status table."""
        import json
        from pathlib import Path as _Path

        empty_summary = html.Small("No pipeline registry loaded.", className="text-muted")

        if not registry_path:
            # Try the default from config
            try:
                from config.config import PIPELINE_REGISTRY
                registry_path = PIPELINE_REGISTRY
        ...
        try:
            ...
            return [], html.Small(
                f"Could not read: {registry_path}", className="text-danger",
            )

        experiments = data.get("experiments", [])
        if not experiments:
            return [], html.Small("Registry is empty.", className="text-muted")
        ...
        for exp in experiments:
            table_data.append({
                ...
                "sex": exp.get("sex", ""),
                "current_stage": exp.get("current_stage", "unknown") or "unknown",
                "last_updated": exp.get("last_updated", ""),
                "has_errors": "Yes" if exp.get("has_errors") else "No",
            })

        # Build summary
        stage_counts = {}
        error_count = sum(1 for e in experiments if e.get("has_errors"))
        for exp in experiments:
        ...
        stage_colors = {
            "acquired": "secondary",
            "copied_to_network": "primary",
            "tracked": "info",
            "processed": "success",
            "synced_to_network": "success",
        }
        for stage, count in sorted(stage_counts.items()):
            color = stage_colors.get(stage, "warning")
        ...
        if error_count:
            badges.append(
                html.Span(f"Errors: {error_count}", className="badge bg-danger me-2")
            )

        summary = html.Div([
            html.Strong(f"Total: {len(experiments)}  "),
            *badges,
            html.Small(
