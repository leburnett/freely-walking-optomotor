...
function f = plot_mean_sem_overlap_2speeds_v19(DATA, strain, landing, sex, data_type)

    % % Eventually have this as the input to the function 
    data = DATA.(strain).(landing).(sex); 

    params =[60, 4, 15; % 1 - 60 deg gratings 
            60, 8, 15; % 2
            1, 4, 15; % 3 - ON curtain
            1, 8, 15; % 4 
            0, 4, 15; % 5 - OFF curtain
            0, 8, 15; % 6 
            21, 4, 15; % 7 - 2ON 14OFF grating 
            21, 8, 15; % 8 
            20, 4, 15; % 9 -2OFF 14ON grating
            20, 8, 15; % 10
            ];
    ...
    for idx2 = 1:1:n_cond
        ...
        if isfield(data, rep1_str)
            ...
            if ismember(idx2, [1,2]) % 60 deg gratings - grey
                col = [0.5 0.5 0.5];
            elseif ismember(idx2, [3,4]) % ON curtains - light red
                col = [1 0.6 0.6]; 
            elseif ismember(idx2, [5,6]) % OFF curtains - light blue
                col = [0.4 0.8 1];
            elseif ismember(idx2, [7,8]) % ON 2:14 - dark red
                col = [0.8 0 0];
            elseif ismember(idx2, [9, 10]) % OFF 2:14 - dark blue
                col = [0 0 0.6];
            end 
            ...
            if idx2>n_cond-2
                ...
                plot([750 750], rng, 'k', 'LineWidth', 0.5)
            ...
            title(strcat(string(p(2)), 'Hz'), 'FontSize', 11)
            if subpl <3
