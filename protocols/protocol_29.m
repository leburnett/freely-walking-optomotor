...
all_conditions = [ 
    9, 47, 127, 1, 15, t_interval, 2; %  60 deg gratings - 4Hz - 1 px step pattern
    27, 47, 127, 1, 15, t_interval, 2; % 60 deg gratings - 8Hz - 2px step pattern
    17, 47, 127, 1, 15, t_interval, 2; % 2:14 ON bars - 4Hz
    24, 47, 127, 1, 15, t_interval, 2; % 2:14 OFF bars - 4Hz
    51, 47, 127, 1, 15, t_interval, 2; % ON curtains - 8Hz - 32 px
    52, 47, 127, 1, 15, t_interval, 2; % OFF curtains - 8Hz - 32 px
    32, 47, 16, 1, 15, t_interval, 2;  % Reverse Phi - 4px step - 4Hz
    32, 47, 32, 1, 15, t_interval, 2;  % Reverse Phi - 4px step - 8Hz
    10, 47, 8, 1, 15, t_interval, 1;  % Flicker - 4Hz
    10, 47, 0, 1, 15, t_interval, 1; % static grating - 60deg
    21, 47, 127, 1, 15, t_interval, 2; % shifted centre of rotation - 60deg - 0.8
    57, 47, 1, 1, 30, t_interval 1; % bar fixation - 32px ON - single bar
];  
    ...
    %start LOOP
     for i = 1:num_conditions
        ...
        elseif current_condition == 5 || current_condition == 6
            Log = present_optomotor_stimulus_curtain_w_interval(current_condition, all_conditions, vidobj, d);
            fieldName = sprintf('log_%d', log_n);
            LOG.(fieldName) = Log;
        else
            Log = present_optomotor_stimulus_w_interval(current_condition, all_conditions, vidobj, d);
